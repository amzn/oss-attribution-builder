...
describe('known licenses', function () {
  ...
  it('declare required properties', function () {
    for (const [name, data] of (licenses as any).entries()) {
      ...
      expect(List.isList(data.get('tags'))).toBe(true, `license ${name}'s tags property was not a list`);
      expect(typeof data.get('text')).toEqual('string', `license ${name}'s text was not a string`);
    }
  });
